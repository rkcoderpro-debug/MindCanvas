...
describe("Workspace UI", () => {
  ...
  it("shows contextual page help on the first visit without completing a guide", async () => {
    ...
    await act(async () => { help.click(); await new Promise(resolve => window.requestAnimationFrame(resolve)); });
  ...
  it("opens V4.8.2 quick search and finds text stored inside a canvas", async () => {
    ...
    cacheProject(null, { board, id: board.id, title: board.title, updatedAt: board.updatedAt, folderId: null, pending: false });
    await act(async () => root.render(<App/>));
    expect(host.querySelector(".beta")?.textContent).toBe(APP_VERSION_LABEL);
