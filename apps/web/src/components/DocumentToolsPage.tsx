...
export default function DocumentToolsPage({ owner, documents, onDocumentsChanged, onBack }: Props) {
  ...
  return <section className="document-tools-page document-tools-shell">
    <header className="tool-page-heading tool-page-hero"><div className="tool-page-heading-copy"><button className="text-button document-tools-back" onClick={onBack}><ArrowLeft size={15}/> Công cụ</button><span className="eyebrow"><Wrench size={15}/> CÔNG CỤ</span><h1>Editor tài liệu</h1><p>Vẽ trên PDF và soạn thảo DOCX từ cùng thư viện tài liệu cục bộ.</p></div><div className="tool-page-heading-actions"><input ref={input} hidden type="file" accept=".pdf,.docx,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document" onChange={event => { void upload(event.target.files?.[0]); event.currentTarget.value = ""; }}/><button className="primary-button" disabled={uploading} onClick={() => input.current?.click()}><Upload size={16}/>{uploading ? "Đang tải…" : "Tải PDF/DOCX"}</button></div></header>
