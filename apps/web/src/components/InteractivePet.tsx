...
export default function InteractivePet({ kind, mood, name, followPointer = true }: { kind: PetKind; mood: PetMood; name: string; followPointer?: boolean }) {
  ...
  useEffect(() => {
    ...
    const move = (event: PointerEvent) => {
      ...
      frame = requestAnimationFrame(() => {
        ...
        const radius = Math.max(190, Math.max(box.width, box.height) * 1.65);
  ...
  const darkFur = kind === 'fox' ? '#c96f42' : kind === 'dog' ? '#947455' : kind === 'rabbit' ? '#c5b3d5' : '#c99a70';
  return <div ref={root} className={`interactive-pet ${bow ? 'bowing' : ''} ${petting ? 'petted' : ''} ${!followPointer || !pointerNear ? 'idle' : ''}`} data-mood={mood} data-follow={followPointer ? 'true' : 'false'} data-pointer-near={pointerNear ? 'true' : 'false'}>
    <svg viewBox="0 0 140 150" aria-hidden="true"><ellipse cx="70" cy="138" rx="46" ry="7" fill="#000" opacity=".1"/>
      ...
      <g className="pet-head">
        ...
        <g fill="none" stroke="#80535b" strokeWidth="1.1" strokeLinecap="round" opacity=".55"><path d="M50 88L30 85 M50 91L28 92 M90 88L110 85 M90 91L112 92"/></g>
