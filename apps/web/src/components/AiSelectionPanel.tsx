...
export default function AiSelectionPanel({ sourceText, canUse, onClose, onApply }: {
  ...
  const clearManual = () => { setManualPrompt(""); setManualJson(""); setManualCopied(false); };
  ...
  const validateManualResult = () => {
    try { setResult(parseManualSelectionResult(manualJson, action)); setError(""); }
    catch { setResult(null); setError(t("manualInvalidResult")); }
  };
  ...
  return <Dialog title={t("aiSelectionTitle")} onClose={close}>
    ...
    {aiMode === "manual" ? <p className="ai-manual-note">{t("aiManualHint")} {t("manualNoLoginHint")}</p> : !canUse && <p className="form-error" role="alert">{t("loginRequired")}</p>}
