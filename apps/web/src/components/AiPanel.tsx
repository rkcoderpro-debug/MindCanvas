...
export default function AiPanel({ projectId, canUse, beforeGenerate, onClose, onApply }: {
  ...
  const { t } = useLanguage();
  const [file, setFile] = useState<File | null>(null), [graph, setGraph] = useState<StructuredMindMap | null>(null), [rawText, setRawText] = useState(""), [pageCount, setPageCount] = useState(0), [documentId, setDocumentId] = useState("");
  ...
  const run = async () => {
    ...
    try {
      ...
      setRawText(doc.text); setPageCount(doc.pageCount ?? 0); setFrom(1); setTo(doc.pageCount ?? 1); setDocumentId(doc.id);
  ...
  return <Dialog title={t("ai")} onClose={onClose}>
    ...
    <label className="upload-drop">{t("choosePdf")}<input type="file" accept=".pdf,application/pdf" disabled={busy || !canUse} onChange={e => { const next=e.target.files?.[0] ?? null; setFile(next); setGraph(null); setRawText(""); setPageCount(0); setDocumentId(""); setError(""); }}/><small>{t("pdfHint")}</small></label>
