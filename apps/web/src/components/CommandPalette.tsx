...
export default function CommandPalette({ projects, onClose, onOpenProject, onCreateProject, onOpenFlashcards, onOpenSettings, onImport }: {
  ...
  const run = (action: () => void) => { onClose(); emitGuideAction("ai:action"); action(); };
  return <Dialog title={t("commandPalette")} onClose={onClose}>
    <label className="command-search"><Search size={19}/><input autoFocus aria-label={t("commandSearch")} placeholder={t("commandSearch")} value={query} onChange={event => { setQuery(event.target.value); if (event.target.value.trim()) emitGuideAction("ai:search"); }}/><kbd>⌘ K</kbd></label>
