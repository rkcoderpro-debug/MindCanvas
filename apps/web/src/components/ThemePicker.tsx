  ...
  return <div className="theme-picker" role="group" aria-label={t("themeChoose")}>
    {(["light", "dark"] as const).map(tone => <section className="theme-group" key={tone} aria-labelledby={`theme-group-${tone}`}>
      ...
      <div className="theme-options-grid">{THEME_OPTIONS.filter(option => option.tone === tone).map(option => {
        ...
        return <button key={option.id} type="button" className={`theme-option ${selected ? "selected" : ""} ${locked ? "locked" : ""} ${option.access === "plus" ? "premium" : ""} ${previewTheme === option.id ? "previewing" : ""}`} aria-pressed={selected} aria-disabled={locked} title={locked ? t("themeLockedHint") : undefined} onPointerEnter={() => preview(option.id)} onPointerLeave={clearPreview} onFocus={() => preview(option.id)} onBlur={event => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) clearPreview(); }} onClick={() => locked ? onLocked?.() : onChange(option.id)}>
