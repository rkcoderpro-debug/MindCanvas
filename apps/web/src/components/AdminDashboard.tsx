...
export default function AdminDashboard({ onBack }: { onBack: () => void }) {
  ...
  const selectUser = async (user: AdminUserSummary) => {
    ...
    try {
      ...
      setDetail(next); setDraftPlan(next.plan.id); setDraftExpiresAt(next.plan.expiresAt ? next.plan.expiresAt.slice(0, 10) : ""); setDraftNote(next.plan.note ?? "");
    } catch (err) { setError(err instanceof Error ? err.message : t("adminLoadError")); }
  ...
  const save = async () => {
    ...
    try {
      await assignAdminPlan(detail.user.userId, { planId: draftPlan, expiresAt: draftExpiresAt ? new Date(`${draftExpiresAt}T23:59:59.000Z`).toISOString() : null, note: draftNote.trim() || null });
      const next = await getAdminUserDetail(detail.user.userId);
      setDetail(next); setDraftPlan(next.plan.id); setDraftExpiresAt(next.plan.expiresAt ? next.plan.expiresAt.slice(0, 10) : ""); setDraftNote(next.plan.note ?? ""); setSaved(true); await load();
