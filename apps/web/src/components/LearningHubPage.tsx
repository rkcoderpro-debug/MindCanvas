...
import { isStudyDayComplete, studyPlanDayFor, vietnamStudyDate, type StudyPlanDay, type StudyPlanMode, type StudyPlanTask, type StudyTaskKind } from "../lib/flashcards";
...
export default function LearningHubPage({ owner, projects, accountPlan }: { owner: string | null; projects: Project[]; accountPlan?: AccountPlan }) {
  ...
  return <section className="learning-hub-page">
    ...
    {tab === "overview" && <HubOverview flashcards={flashcards} quizzes={quizzes} onTab={setTab} t={t} language={language}/>}
    ...
    {tab === "plan" && <StudyPlannerPanel flashcards={flashcards} quizzes={quizzes} language={language} t={t}/>}
    {tab === "progress" && <ProgressPanel flashcards={flashcards} quizzes={quizzes} language={language} t={t}/>}
  </section>;
}

function HubOverview({ flashcards, quizzes, onTab, t, language }: { flashcards: FlashcardStore; quizzes: QuizStore; onTab: (tab: HubTab) => void; t: ReturnType<typeof useLanguage>["t"]; language: string }) {
  const plan = flashcards.activeStudyPlan;
  const today = vietnamStudyDate();
  const planDay = plan ? studyPlanDayFor(plan, today) : null;
  ...
  return <div className="learning-hub-overview">
    <div className="learning-hero"><div className="learning-hero-copy"><span className="eyebrow">{plan ? t("todayPlan") : t("quickStart")}</span><h2>{plan ? plan.name : t("continueLearning")}</h2><p>{plan ? t("manualPlanStreakHint") : t("noPlanFallbackHint")}</p><button className="primary-button" onClick={() => onTab(plan ? "plan" : "flashcards")}><Sparkles size={17}/>{plan ? t("openTodayPlan") : t("startLearning")}</button></div><div className="learning-hero-orbit"><Trophy size={32}/><strong>{flashcards.streak.current}</strong><span>{t("streakDays")}</span></div></div>
...
function StudyPlannerPanel({ flashcards, quizzes, language, t }: { flashcards: FlashcardStore; quizzes: QuizStore; language: string; t: ReturnType<typeof useLanguage>["t"] }) {
