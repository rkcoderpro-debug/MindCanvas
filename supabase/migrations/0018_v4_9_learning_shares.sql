...
create table public.learning_share_invitations (
...
create index learning_invites_owner on public.learning_share_invitations(owner_id,kind,resource_id);
create index learning_invites_email on public.learning_share_invitations(email,status);

create table public.learning_share_members (
...
create index learning_members_recipient on public.learning_share_members(user_id);
...
returns boolean language sql stable security definer set search_path = '' as $$
  select exists(select 1 from public.learning_share_members m
    where m.kind=p_kind and m.resource_id=p_id and m.user_id=auth.uid()
      ...
      and public.learning_share_tier(m.owner_id) >= case when p_kind='lab' then 2 else 1 end)
$$;
...
create or replace function public.learning_bump_version() returns trigger language plpgsql as $$
begin
...
begin
  ...
  if public.learning_share_tier(auth.uid()) < case when p_kind='lab' then 2 else 1 end then raise exception 'Upgrade your plan to share this material'; end if;
...
begin
  ...
  if not public.learning_resource_owner(v.kind,v.resource_id,v.owner_id) or public.learning_share_tier(v.owner_id)<case when v.kind='lab' then 2 else 1 end then raise exception 'Sharing is paused or material removed'; end if;
...
create index learning_quiz_attempts_user on public.learning_quiz_attempts(user_id,quiz_id);
...
begin
  ...
  if jsonb_typeof(p_answers)<>'array' or jsonb_array_length(p_answers)<>v_total then raise exception 'Invalid answers'; end if;
...
create table public.learning_card_progress (
  user_id uuid not null references auth.users(id) on delete cascade,
...
begin
  ...
  if c.id is null or not public.learning_share_allowed('flashcard',c.deck_id) or p_rating not in ('again','hard','good','easy') then raise exception 'Card unavailable or access revoked'; end if;
...
language sql stable security definer set search_path = '' as $$
  select m.kind,m.resource_id,coalesce(p.display_name,'Người chia sẻ')::text,
    ...
    case when not public.learning_resource_owner(m.kind,m.resource_id,m.owner_id) then 'removed'
      when public.learning_share_tier(m.owner_id)<case when m.kind='lab' then 2 else 1 end then 'paused'
