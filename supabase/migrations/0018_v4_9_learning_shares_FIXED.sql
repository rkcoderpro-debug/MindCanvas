...
-- Shared Quiz must go through security-definer RPCs; direct SELECT exposes
-- correctIndex and explanation before submit (see migration 0020).
...
-- Do not expose the private question snapshot (correctIndex/explanation) before
-- submit. Migration 0020 also removes this policy; re-running 0018 must not
-- restore it.
...
-- CREATE OR REPLACE cannot change a function's return type (42P13). Only
-- remove the legacy integer-returning overload; leave a newer jsonb function
-- in place so this migration remains safe to re-run after 0020/0021.
do $$
declare existing_function regprocedure := to_regprocedure('public.finish_learning_quiz(uuid,jsonb)');
begin
  if existing_function is not null and
     (select p.prorettype from pg_proc p where p.oid=existing_function) <> 'jsonb'::regtype then
    drop function public.finish_learning_quiz(uuid,jsonb);
  end if;
end $$;
...
begin
  ...
  -- The optional immediate-feedback migration records each revealed choice.
  -- Keep those choices authoritative even if 0018 is re-run after it.
  if to_jsonb(a)->>'answer_reveal_mode' = 'instant' and a.answers is distinct from p_answers then
...
grant select on public.learning_share_invitations,public.learning_share_members,
  public.learning_card_progress to authenticated;
revoke select on public.learning_quiz_attempts from authenticated;
